...
if (isset($_POST['proses-kepangkatan'])) {
   ...
   @$id = $_POST['id'];
   if ($control == "add") {
      $sql =  mysqli_query($koneksi, "INSERT INTO kepangkatan (nik, pangkat, golongan, nomor_sk, tanggal_sk, terhitung_mulai)VALUES('$nik','$pangkat','$golongan','$nomor_sk','$tanggal_sk','$terhitung_mulai')");
   } else if ($control == "delete") {
      $sql = mysqli_query($koneksi, "DELETE FROM inpasing WHERE id='$id' ");
   }
   if ($sql) {
   ...
   $_SESSION['redirectlogin'] = 'module/user/kepangkatan';
}

if (isset($_POST['upload-kepangkatan'])) {
   $tipe = "Inpasing";
   $nik = $_POST['nik'];
